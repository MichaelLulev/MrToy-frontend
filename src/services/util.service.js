...
export function getPaths(object, value, maxDepth=3, checkEqual=((a, b) => a === b), path='', paths=[], origObj=null) {
    if (maxDepth < 0 || object === origObj) return
    for (const key in object) {
        const _path = path + `.${key}`
        if (checkEqual(object[key], value)) paths.push(_path)
        if (typeof object[key] !== 'object') continue
        getPaths(object[key], value, maxDepth-1, checkEqual, _path, paths, object)
    }
    return paths
}